...
class SimGCL(GraphRecommender):
    def __init__(self, conf, training_set, val_set, test_set, **kwargs):
        super(SimGCL, self).__init__(conf, training_set, val_set, test_set,**kwargs)
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            for n, batch in enumerate(self.sample_function(self.data, self.batch_size)):
                ...
                reg_loss = l2_reg_loss(self.reg, user_emb, pos_item_emb)
                batch_loss =  cl_loss + reg_loss + rec_loss
                ...
                if n % 20==0:
                    ...
                    if self.config.contain('is_wandb') and self.config['is_wandb']=='True':
                        wandb.log({"rec_loss": rec_loss.item(), "cl_loss": cl_loss.item(), "l2_reg_loss": reg_loss})
            with torch.no_grad():
